...
export const projectData = [
{
    ...
    role: [     
      "게시글 CRUD와 댓글, 검색 기능을 구현",
      "Axios를 활용해 백엔드와 REST API 통신을 진행",
      "MySQL 데이터베이스 기반으로 게시글 데이터관리",
      "페이징 처리를 적용해 데이터 로딩 성능을 개선",
      "이미지 첨부 기능을 추가하여, 이미지 미등록 시 기본 이미지가 자동으로 표시되도록 구현",
      "게시글 상세페이지에 조회수, 게시글 번호, 작성일 등 메타데이터를 표시해 사용자 편의성을 향상",
      "권한별 접근 제어를 통해 일반 사용자는 자신의 게시글만 수정 및 삭제 가능, 관리자는 전체 게시글 삭제 가능하도록 설계"
    ],
    details: [
...
},
{
    ...
    role: [     
      "사용자가 맛집에 대한 리뷰를 작성",
      "리뷰작성 시 텍스트 입력뿐 아니라 사진 첨부 및 별점 평가 기능을 추가하여 실제 사용자 후기처럼 생생하게 표현될 수 있도록 구현",
      "이메일을 기준으로 중복가입이 되지 않도록 MySQL 연동",
      "프로필 이미지는 Firebase Storage를 통해 관리하여 사용자의 정보 접근성과 보안성을 향상",
      "로그인 상태에 따라 네브바 화면 구성이 다르게 표시되도록 구현(비로그인, 일반 회원, 관리자 상태를 구분하여 각각의 접근 권한에 맞는 메뉴만 노출) ",
      "관리자 계정으로 로그인 시 새로운 맛집 정보를 직접 등록할 수 있는 기능을 앱 내 컨텐츠 관리가 용이하도록 설계",
      "일러스트를 활용하여 앱과 어울리는 캐릭터 및 앱명 제작"
    ],
...
{
    id: 3,
    title: "머신러닝을 활용한 빅데이터 프로젝트",
    thumbnail: home3,
    skills: [tableau, github, python, react, spring],
    shortDesc: "112 신고 데이터 기반 범죄 안전도 예측",
    period: "2023.09.18~2023.10.23",
    members: "4명",
    description: "서울 전역의 위험도를 머신러닝 기법으로 분석하고, 지역별 데이터 특성을 기반으로 범죄 발생률과의 상관관계를 검증하기 위한 빅데이터 프로젝트",
    role: [     
      "서울 전 지역 데이터를 확보하는 과정에서 일부 지역의 데이터 부족 문제를 해결하기 위해 Naver Reverse Geocoding API를 활용하여 Polygon 좌표(위도,경도)기반으로 행정동 정보를 추출",
      "데이터의 신뢰성과 균형성을 확보하기 위해 서울시를 4개 구역(영등포구, 강서구, 강남구, 강동구)으로 구분하고 영등포구 데이터를 중심으로 분석을 담당",
      "영등포구 관련 데이터들을 수집 후 이상치 제거 및 정규화를 하여 머신러닝 분석에 적합한 형태로 가공",
      "Spring Boot와 React를 연동하여 지역별 위험도 시각화 대시보드 제작 (지역별 세부적으로 항목을 구분)"
    ],
